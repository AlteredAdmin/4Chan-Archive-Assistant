...
def download_file(url, folder):
    ...
    if os.path.exists(local_filename):
        ...
        return local_filename, False  # Return the filename and False indicating the file was not downloaded
    ...
    with requests.get(url, stream=True) as response:
        ...
        with open(local_filename, 'wb') as f:
            for chunk in response.iter_content(chunk_size=8192):
                f.write(chunk)
    return local_filename, True  # Return the filename and True indicating the file was downloaded
...
def download_from_thread(thread_url):
    ...
    title = thread.topic.subject if thread.topic.subject else ' '.join(thread.topic.comment.split()[:5])
    ...
    for post in thread.all_posts:
        if post.has_file:
            _, downloaded = download_file(post.file_url, full_save_path)
            if downloaded:
